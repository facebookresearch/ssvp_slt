...
def temporal_sampling(
    ...
    index = torch.linspace(
        start_idx + 0.5, end_idx + 0.5, num_samples, device=frames.device
    )
...
def normalize_magnitude(magnitude: np.ndarray) -> np.ndarray:
    ...
    return (
        ...
        * (magnitude - magnitude.min())
        / max(float(magnitude.max() - magnitude.min()), 1)
    ).astype(np.uint8)
...
def find_target_bbox(
    ...
    while total_bboxes > 0:
        anchor = next(
            (i, bbox_arr[i].pop()) for i, bboxes in enumerate(bbox_arr) if bboxes
        )
        tube = [anchor]
    ...
    target_bbox = (
        tuple(np.array([bbox[1] for bbox in best_tube]).mean(axis=0))
        if best_tube
        else None
    )
...
def calculate_mean_val(tube: Tube, opts: List[np.ndarray]) -> float:
    ...
    mean_val = sum(
        ...
        for frame_index, (x0, y0, x1, y1) in (
            (x[0], tuple(map(int, x[1]))) for x in tube
        )
    )
...
def calculate_bbox_from_tubes(tubes: Sequence[Tube]) -> Optional[BboxTuple]:
    ...
    """
    total_sizes = [
        sum((bbox[3] - bbox[0]) * (bbox[2] - bbox[1]) for _, bbox in tube)
        for tube in tubes
    ]
...
def crop_resize(
    imgs: Sequence[np.ndarray], bbox: BboxTuple, target_size: int
) -> np.ndarray:
    """
...
class Preprocessor:
    ...
    def _bbox_from_bboxes_mean(
        self, bboxes: Sequence[Sequence[BboxTuple]]
    ) -> BboxTuple:
        """
    ...
    def _sample_frames_for_feature_extraction(
        ...
        for i in range(0, frames.shape[0], stride * sampling_rate):
            ...
            num_padding_frames_list.append(
                get_num_padding_frames(
                    idx, num_frames, sampling_rate, fps, self.config.target_fps
                )
            )
        ...
        new_frames = tensor_normalize(
            new_frames, tuple(self.config.mean), tuple(self.config.std)
        )
    ...
    def __call__(self, video_path: Path) -> Dict[str, torch.Tensor]:
        ...
        bboxes, max_num_faces = self.detect_faces(
            frames[:: self.config.detection_sampling_rate]
        )
        ...
        if self.config.verbose:
            print(f"Preprocessing: {t5 - t0:.3f}s")
