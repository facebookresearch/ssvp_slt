...
"""
Utils for SignHiera
Adapted from https://github.com/facebookresearch/hiera/blob/v0.1.2/hiera/hiera_utils.py
"""
...
def pretrained_model(checkpoints: Dict[str, str], default: str = None) -> Callable:
    """
    Loads a SignHiera model from a pretrained source (if pretrained=True).
    Use "checkpoint" to specify the checkpoint.
    """
            ...
            **kwdargs,
            ...
            if pretrained:
                ...
                if "head.projection.weight" in state_dict["model_state"]:
                    ...
                    # If the user specified a different number of classes,
                    # remove the projection weights or else we'll error out
                    elif (
                        kwdargs["num_classes"]
