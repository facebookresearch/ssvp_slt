...
SIGNHIERA_EXPECTED_EXTRA = ("decoder", "multi_scale_fusion", "encoder_norm", "mask_token")


def load_model(model: torch.nn.Module, checkpoint_path: Path, model_key: str = "model") -> None:
    """
    ...
    missing_keys, unexpected_keys = model.load_state_dict(new_checkpoint_model, strict=False)
    ...
    unexpected_keys = [k for k in unexpected_keys if not k.startswith(SIGNHIERA_EXPECTED_EXTRA)]
...
FLORES200_ID2LANG = {v: k for k, v in FLORES200_LANG2ID.items()}
